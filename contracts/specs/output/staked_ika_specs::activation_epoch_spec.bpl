...
// fun staked_ika_specs::activation_epoch_spec [verification] at ./sources/staked_ika_specs.move:16:1+94
procedure {:timeLimit 3000} $0_staked_ika_activation_epoch$verify(_$t0: $0_staked_ika_StakedIka) returns ($ret0: int)
{
    // declare local variables
    var $t1: int;
    var $t2: int;
    ...
    // bytecode translation starts here
    // assume WellFormed($t0) at ./sources/staked_ika_specs.move:16:1+1
    ...
    call $t1 := $0_staked_ika_activation_epoch$impl($t0);
    if ($abort_flag) {
        assume {:print "$at(255,491,523)"} true;
        $t2 := $abort_code;
...
// fun staked_ika_specs::activation_epoch_spec [baseline] at ./sources/staked_ika_specs.move:16:1+94
procedure {:inline 1} $0_staked_ika_activation_epoch$asserts(_$t0: $0_staked_ika_StakedIka) returns ()
{
    // declare local variables
    var $t1: int;
    var $t2: int;
...
// fun staked_ika_specs::activation_epoch_spec [verification] at ./sources/staked_ika_specs.move:16:1+94
procedure {:timeLimit 3000} $0_staked_ika_activation_epoch$spec_no_abort_check$verify(_$t0: $0_staked_ika_StakedIka) returns ()
{
    // declare local variables
    var $t1: int;
    var $t2: int;
    ...
    // $t1 := staked_ika::activation_epoch($t0) at ./sources/staked_ika_specs.move:17:5+32
    assume {:print "$at(255,491,523)"} true;
    $t1 := $0_staked_ika_activation_epoch$opaque($t0);

    // $t1 := havoc[val]() at ./sources/staked_ika_specs.move:16:1+94
    assume {:print "$at(255,431,525)"} true;
    havoc $t1;
