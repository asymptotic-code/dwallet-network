...
// fun staked_ika_specs::join_spec [verification] at ./sources/staked_ika_specs.move:36:1+94
procedure {:timeLimit 3000} $0_staked_ika_join$verify(_$t0: $Mutation ($0_staked_ika_StakedIka), _$t1: $0_staked_ika_StakedIka) returns ($ret0: $Mutation ($0_staked_ika_StakedIka))
{
    // declare local variables
    var $t2: int;
    var $t0: $Mutation ($0_staked_ika_StakedIka);
    ...
    // bytecode translation starts here
    // assume WellFormed($t0) at ./sources/staked_ika_specs.move:36:1+1
    assume {:print "$at(255,1044,1045)"} true;
    assume $IsValid'$0_staked_ika_StakedIka'($Dereference($t0));

    // assume WellFormed($t1) at ./sources/staked_ika_specs.move:36:1+1
    assume $IsValid'$0_staked_ika_StakedIka'($t1);
    ...
    call $t0 := $0_staked_ika_join$impl($t0, $t1);
    if ($abort_flag) {
        assume {:print "$at(255,1109,1136)"} true;
        $t2 := $abort_code;
...
// fun staked_ika_specs::join_spec [baseline] at ./sources/staked_ika_specs.move:36:1+94
procedure {:inline 1} $0_staked_ika_join$asserts(_$t0: $Mutation ($0_staked_ika_StakedIka), _$t1: $0_staked_ika_StakedIka) returns ()
{
    // declare local variables
    var $t2: int;
    var $t0: $Mutation ($0_staked_ika_StakedIka);
...
// fun staked_ika_specs::join_spec [verification] at ./sources/staked_ika_specs.move:36:1+94
procedure {:timeLimit 3000} $0_staked_ika_join$spec_no_abort_check$verify(_$t0: $Mutation ($0_staked_ika_StakedIka), _$t1: $0_staked_ika_StakedIka) returns ()
{
    // declare local variables
    var $t2: int;
    var $t0: $Mutation ($0_staked_ika_StakedIka);
    ...
    // staked_ika::join($t0, $t1) at ./sources/staked_ika_specs.move:37:5+27
    assume {:print "$at(255,1109,1136)"} true;
    $t0 := $0_staked_ika_join$opaque($t0, $t1);

    // $t0 := havoc[mut]() at ./sources/staked_ika_specs.move:36:1+94
    assume {:print "$at(255,1044,1138)"} true;
